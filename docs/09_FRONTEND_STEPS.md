...
|---|---|---|
| **FE-T00** | **Close the backend gaps in §13.5** | Eight items, B-01…B-08. **Hard blockers for FE-T02:** **B-01** (CORS/proxy — nothing can call the API until resolved), **B-04** (token storage decision). **Blockers for a specific later task only:** B-03 (auth flow → FE-T03), B-05 (payslip PDF → FE-T31), B-06 (lock flags → FE-T19/T20/T22 fall back to R-06b), B-07 (2 missing tax masters → FE-T24 scope), B-08 (pagination → §14 R-08 workaround). **Not a blocker, just confirm:** B-02. Also raise the `/payslip-components` read-role gap (§15.6) and the missing `/users` PUT/DELETE (§15.14). Do not start FE-T02 with B-01 open. |
| FE-T01 | Scaffold `apps/web` | Replace the Vite starter (`App.tsx`/`App.css`/assets). Add `react-router-dom` + `axios` (§13.1 — the complete sanctioned addition list). Create the §6 folder skeleton: `pages/ features/ components/ api/ hooks/ routes/`. Wire `QueryClientProvider` (one client, app-wide defaults) + `ConfigProvider` theme tokens + `RouterProvider` per §13.2. |
| FE-T02 | Typed API client + error contract | One axios instance (`baseURL` per B-02) with a request interceptor for `Authorization` and a response interceptor per §14 R-04. Build `src/api/errors.ts` → `describeApiError()` with the full status table, the 400 `message[]` → antd `Form` field mapping, and the 409 spec-reference stripper (`§11`, `TC-…`, `P8-T0…`). **This module is the single most reused piece in the app — get it right before any screen exists.** |
| FE-T03 | Auth: login, session, guards | `/login` (antd `Form`), `AuthProvider` context (`user`, `isAdmin`, `login`, `logout`), token persistence per the B-04 decision, 401 → logout+redirect, 403 → forbidden page **without** logout. Session restore on reload works with **no** `/auth/me` (§13.5 B-03) — do not build against a refresh endpoint that doesn't exist. |
| FE-T04 | App shell + access map | antd `Layout`/`Sider`/`Header`, breadcrumbs, user menu. `src/routes/access.ts` — the one map feeding both nav filtering and route guards (§14 R-11), populated from §15.15. `/403` and `*` pages. |
| FE-T05 | Shared UI kit | The four archetypes in §15.0 (`ListPage`, `DetailPage`, `FormDrawer`, `EffectiveDatedMasterPage`), plus: `<LockedAction>` (disabled control + reason tooltip, §14 R-06), `<StatusTag>` (exhaustive `Record<Enum,…>` label+color maps, §14 R-05), `formatIDR()` (format only — never derive, §14 R-07), `useDownloadPdf()` (authenticated blob download, §15.10), and the standard loading/empty/error wrappers. **Every later task composes these; none re-implements them.** |
