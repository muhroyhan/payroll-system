...
// call: on 401, clear the session and redirect (no refresh endpoint exists,
// B-03, so there is nothing to retry). Every other status is left for the
// caller to interpret via describeApiError() at the point it's rendered —
// the interceptor does not transform or swallow the rejection.
apiClient.interceptors.response.use(
  (response) => response,
  (error: unknown) => {
    const presentation = describeApiError(error);
    if (presentation.kind === 'auth' && window.location.pathname !== '/login') {
      clearSession();
      markSessionExpired();
