...
// FE-T09 (09_FRONTEND_STEPS.md), R-13 (07_FRONTEND_RULES.md) — displays
...
export function SalaryResolvePreview() {
  ...
  const resolveQuery = useResolveSalaryQuery(employeeId, asOf.format('YYYY-MM-DD'));
      ...
      {employeeId && (
        <div style={{ marginTop: 12 }}>
          ...
          {!resolveQuery.isLoading && resolveQuery.data?.resolved && (
            <Typography.Text>
              {formatIDR(Number(resolveQuery.data.record.baseSalary))}{' '}
          ...
          {!resolveQuery.isLoading && resolveQuery.data && !resolveQuery.data.resolved && (
            <Typography.Text type="secondary">
              Tidak ada aturan gaji yang berlaku untuk kombinasi ini.
            </Typography.Text>
          )}
        </div>
      )}
